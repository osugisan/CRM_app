                                  ...
                                  <table class="table-auto w-full text-left whitespace-no-wrap">
                                      ...
                                      <tbody>
                                          <tr v-for="order in props.orders.data" :key="order.id">
                                              ...
                                              <td class="border-t-2 border-b-2 border-gray-200px-4 py-3">{{ order.customer_name }}</td>
                                              <td class="border-t-2 border-b-2 border-gray-200px-4 py-3">{{ order.total }}</td>
                                              <td class="border-t-2 border-b-2 border-gray-200px-4 py-3">
                                                <div v-if="order.status === 0">キャンセル済</div>
                                            </td>
                                              <td class="border-t-2 border-b-2 border-gray-200px-4 py-3">{{ dayjs(order.created_at).format('YYYY-MM-DD HH:mm:ss') }}</td>
                                          </tr>
                                      </tbody>
                                  </table>
                              </div>
